...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    // - SIGNED_IN via lien invitation (type=invite dans le hash)
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(
      (event, session) => {
        ...
        // Cas 1 : lien d'invitation cliqué → type=invite
        // Supabase émet SIGNED_IN avec user.role="authenticated"
        // et user.app_metadata.provider="email", email_confirmed_at null
        if (event === "SIGNED_IN" && u.app_metadata?.provider === "email" && !u.email_confirmed_at) {
          setView("set-password");
          return;
        }

        // Cas 2 : lien "reset password" par email → PASSWORD_RECOVERY
        if (event === "PASSWORD_RECOVERY") {
          setView("set-password");
          return;
        }

        // Cas 3 : métadonnée posée manuellement (mot de passe temporaire)
        if (u.user_metadata?.need_password_change === true) {
          setView("set-password");
          return;
        }

        // Cas 4 : connexion normale ou MDP déjà changé
        setView("app");
        setShowLogin(false);
      }
    );
  ...
  if (view === "set-password" && user) {
    return (
      <SetPassword
        user={user}
        onSuccess={() => { setView("app"); setShowLogin(false); }}
      />
    );
  }
    ...
    <div className="min-h-screen bg-white">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-10 space-y-8">
        ...
        <div className="max-w-2xl">
          {view === "app" ? (
            <span className="inline-flex items-center gap-1.5 text-xs font-semibold text-emerald-700 bg-emerald-50 border border-emerald-200 px-3 py-1.5 rounded-full mb-4">
        ...
        <div className="relative">
          ...
          <div className={`bg-white border border-slate-100 rounded-2xl p-6 shadow-sm ${view === "teaser" ? "opacity-70 pointer-events-none select-none" : ""}`}>
            <SearchForm disabled={view === "teaser"} onBlockedClick={() => setShowLogin(true)} />
          </div>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full bg-white border border-slate-200 rounded-xl px-4 py-3 pr-10 text-sm placeholder-slate-400 focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/15 shadow-sm transition-all"
                />
            ...
            <div>
              ...
              <input
                ...
                className={`w-full bg-white border rounded-xl px-4 py-3 text-sm placeholder-slate-400 focus:outline-none focus:ring-2 shadow-sm transition-all ${
                  confirm && confirm !== pwd
...
function LoginModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
      ...
      <div className="relative w-full max-w-sm bg-white/95 backdrop-blur-2xl border border-white/70 rounded-3xl shadow-2xl p-8 overflow-hidden">
          ...
          <div className="relative space-y-3">
            ...
            <form onSubmit={handleSignIn} className="space-y-3">
              <input type="email" required value={email} onChange={e => setEmail(e.target.value)}
                ...
                className="w-full bg-white border border-slate-200 rounded-xl px-4 py-3 text-sm placeholder-slate-400 focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/15 shadow-sm transition-all"/>
